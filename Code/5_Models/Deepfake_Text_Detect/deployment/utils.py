...
def preprocess(text, language):
    ...
    text = _clean_text(text, language)
    return text


# This is the original detect function
...
#     tensor_input = torch.tensor([tokenize_input["input_ids"]])
...
# This is the function that we modify the original and write it with Softmax to also calculate Probabilities.
def detect(input_text, tokenizer, model, device='cuda:0'):
    label2decisions = {
        0: "machine-generated",
        1: "human-written",
    }
    ...
    decision_idx = torch.argmax(probabilities, dim=-1).item()
    decision = label2decisions[decision_idx]  # Map index to label

    # Probability of the class 'human-written'
    probability_of_positive_class = probabilities[:, 1].item()
    return decision, probability_of_positive_class

# # This is the function that we modify the original and write it with the original threshold to also calculate Probabilities.
# def detect(input_text, tokenizer, model, device='cuda:0', th=-3.08583984375):
#     label2decisions = {
#         0: "machine-generated",
#         1: "human-written",
...
#     # The threshold applies to a specific logit value; we assume using the logit for 'human-written'
#     # index 1 corresponds to 'human-written'
#     is_human_written = logits[:, 1] > th
#     decision = label2decisions[int(is_human_written.item())]
